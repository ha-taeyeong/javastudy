...
public class Ex03_operator {
	...
	public static void main(String[] args) {
		...
		// 문제1. 평균이 85 이상이거나, 국어와 영어 점수가 모두 80점 이상이면  "합격", 아니면 "불합격"
		int kor = 80;
		int eng = 90;
		boolean isPass = ((kor + eng) / 2 >= 85) || (kor >= 80 && eng >= 80); // 평균구하기를 무조건 하는 버전
//		boolean isPass = ((kor >= 80 && eng >= 80 || (kor + eng) / 2 >= 85)); 평균구하기를 안 할 수도 있는 버전
		...
		int front = square / 100; //4자리 수의 앞에 두자리를 분리하기 위해 100을 나눈 몫
		int end = square % 100; //4자리 수의 뒤에 두자리를 분리하기 위해 100을 나눈 나머지
